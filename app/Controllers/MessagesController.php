...
class MessagesController extends \MainController
{
    ...
    public function get_all_messages(){
        $role = "sender_delete";
        $id = $_SESSION['user_id'];
        $query = DB::run("SELECT user_to FROM " . TBL_MESSAGES . " WHERE id = '" . $message_id . "'");
    ...
    public function get_message($message_id){
        ...
        $id = $_SESSION['user_id'];
        $query = DB::run("SELECT user_to FROM " . TBL_MESSAGES . " WHERE id = '" . $message_id . "'");
        while($data = $data = $query->fetchObject()){
            if($data->user_to != $id){
                $role = "receiver_delete";
            }
        }
        $query = DB::run("SELECT * FROM " . TBL_MESSAGES . " WHERE id = '" . $message_id . "' AND (user_to = '" . $id . "' OR user_from = '" . $id . "') OR respond = '" . $message_id . "' AND " . $role . " != 'n'");
        return $query;
    } // END get_message
    ...
    public function delete_message($message_id){
        $role = "sender_delete";
        $id = $_SESSION['user_id'];
        $query = DB::run("SELECT user_to FROM " . TBL_MESSAGES . " WHERE id = '" . $message_id . "' OR respond = '" . $message_id . "'");
        while($data = $query->fetchObject()){
            if($data->user_to != $id){
                $role = "receiver_delete";
            }
        ...
        $query1 = DB::run("UPDATE " . TBL_MESSAGES . " SET " . $role . " = 'y' WHERE id = '" . $message_id . "'");
    ...
    public function delete_conversation($conversation_id){
        ...
        DB::run("UPDATE " . TBL_MESSAGES . " SET " . $role . " = 'y' WHERE id = '" . $conversation_id . "'");
    }

    private function _check_for_deleted_messages(){
        DB::run("DELETE FROM " . TBL_MESSAGES . " WHERE sender_delete = 'y' AND receiver_delete 'y'"); // removes messages from DB if both sender and receiver have deleted it.
    ...
    private function _email_user_of_new_message($to,$from,$subject){
        $r = DB::run("SELECT first_name,last_name,email FROM " . TBL_USERS . " WHERE id = '" . $to . "'")->fetchObject();
        $u = DB::run("SELECT first_name,last_name FROM " . TBL_USERS . " WHERE id = '" . $from . "'")->fetchObject();
